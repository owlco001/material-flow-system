...
/** Maps failures to safe, actionable copy: local read vs network vs server status. */
object Debug3dErrorPolicy {
    fun upload(error: Throwable): String =
        "上传失败：${message(error, local = "本地文件读取失败，请重新选择 GLB 文件", fallback = "上传暂时不可用")}"

    fun load(error: Throwable): String =
        "加载失败：${message(error, local = "本地文件读取失败，请重试", fallback = "模型下载或校验失败")}"

    /** Pick/cache phase copy: keeps own validation text, never echoes raw I/O messages. */
    fun localFile(error: Throwable): String = when (error) {
        is com.company.logistics.data.remote.LocalFileReadException ->
            "文件校验失败：本地文件读取失败，请重新选择 GLB 文件"
        is IllegalArgumentException -> "文件校验失败：${error.message ?: "所选 GLB 文件无效，请重新选择"}"
        else -> "文件校验失败：所选 GLB 文件无效，请重新选择"
    }

    private fun message(error: Throwable, local: String, fallback: String): String = when (error) {
        ...
        is com.company.logistics.data.remote.LocalFileReadException -> local
