...
// Configure a private HTTPS endpoint in local.properties or a secure build flavor.
private const val API_BASE = "https://configure-your-private-api.invalid"
...
private object MaterialFlowApi {
    ...
    suspend fun orderStatus(token: String, order: String): Result<String> = request("/api/v1/orders/material-status", "POST", token, "{\"documentType\":\"PRODUCTION_ORDER\",\"documentNo\":\"$order\"}").map { root -> val x = JSONObject(root); val items = x.getJSONArray("items"); if (items.length() == 0) "无物料" else items.getJSONObject(0).getString("label") }
    suspend fun createInbound(token: String, material: Material): Result<String> = request("/api/v1/transfer-requests", "POST", token, "{\"clientOperationId\":\"${UUID.randomUUID()}\",\"type\":\"INBOUND\",\"documentNo\":null,\"items\":[{\"materialId\":\"material-id\",\"quantity\":1,\"targetLocationCode\":\"${material.location}\",\"expectedInventoryVersion\":1}],\"remark\":\"入库申请\",\"evidenceIds\":[]}").map { JSONObject(it).getString("status") }
...
@Composable private fun ScanPage(message: String, onMaterial: (String) -> Unit, onQueue: () -> Unit, onOrder: (String) -> Unit) { var code by remember { mutableStateOf("") }; var order by remember { mutableStateOf("") }; Column(verticalArrangement = Arrangement.spacedBy(10.dp)) { Text("扫码作业", style = MaterialTheme.typography.headlineSmall); Text(message); OutlinedTextField(code, { code = it }, Modifier.fillMaxWidth(), label = { Text("料号 / 扫码结果") }); Button({ onMaterial(code) }, Modifier.fillMaxWidth()) { Text("查询料号库存") }; OutlinedTextField(order, { order = it }, Modifier.fillMaxWidth(), label = { Text("生产订单号") }); OutlinedButton({ onOrder(order) }, Modifier.fillMaxWidth()) { Text("查询生产订单物料状态") }; OutlinedButton(onQueue, Modifier.fillMaxWidth()) { Text("查看离线队列") } } }
